...
class DataGenerator(Sequence):
    ...
    def __getitem__(self, idx):
    	batch_x = HDF5Matrix(self.x/"reviews.h5", "x_" + self.flag, start=idx*self.batch_size, end=(idx+1)*self.batch_size).value
    	batch_y = HDF5Matrix(self.y/"reviews.h5", "y_" + self.flag, start=idx*self.batch_size, end=(idx+1)*self.batch_size).value
...
def generate_raw_reviews_json(data_directory, flag = "train", batch_size = 100):
    ...
    for f in files:
        with open(str(f), 'r', encoding = 'utf-8') as file:
            while True:
                text_chunk = []
                for line in list(itertools.islice(file, batch_size)):
                    raw_dict = json.loads(line)
                    raw_dict["x"] = list(map(cleanSentences, raw_dict["x"].split()))
